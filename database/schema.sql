...
CREATE TABLE `sg_product_pincodes` (
    `id`         INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `product_id` INT UNSIGNED NOT NULL,
    `pincode`    VARCHAR(10)  NOT NULL,
    `created_at` DATETIME     NOT NULL DEFAULT CURRENT_TIMESTAMP,
    INDEX `idx_pincode_product` (`product_id`),
    INDEX `idx_pincode_value` (`pincode`),
    CONSTRAINT `fk_pincode_product` FOREIGN KEY (`product_id`) REFERENCES `sg_products`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- +----------------------------------------------------------------------------
-- | SECTION 8 – NAVIGATION, STORE CONFIG, & SEO
-- +----------------------------------------------------------------------------
...
-- +----------------------------------------------------------------------------
-- | SECTION 9 – STORE FRONT & HOMEPAGE
-- +----------------------------------------------------------------------------

CREATE TABLE `sg_deals` (
    `id`          INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `image`       VARCHAR(255) NOT NULL,
    `title`       VARCHAR(200) NULL,
    `link`        VARCHAR(255) NULL,
...
CREATE TABLE `sg_category_cards` (
    `id`            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `title`         VARCHAR(200) NOT NULL,
    `subtitle`      VARCHAR(500) NULL,
